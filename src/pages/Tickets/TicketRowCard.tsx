    ...
    return (
        <div key={ticket.id}
            ...
            <div className={"flex gap-28"}>
                ...
                <p className={"font-IRANSansXRegular flex justify-center w-36  items-center text-onBackground opacity-80"}>{ticket.category}</p>
                <Button disabled={true}
                        className={"bg-primary py-2 w-20 justify-center text-center flex items-center font-IRANSansXRegular rounded-2xl"}>{replaceEnglishDigits(ticket.status)}</Button>
            </div>
        </div>
    )
}